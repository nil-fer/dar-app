...
__webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js"); // require("channels")
...
$(document).ready(function () {
  ...
  window.toggleBookModal = function (element) {
    var modal = element.closest('.product_card_info').querySelector('.backdrop');
    modal.classList.toggle('display-none');
    modal.classList.toggle('display-block');
  };

  window.validateBooking = function (element) {
    if (element.closest('form').checkValidity()) {
      toggleBookModal(element);
    }
  };

  window.toggleSuccessModal = function (element) {
    var modal = element.closest('.backdrop');
    modal.classList.toggle('display-none');
    modal.classList.toggle('display-block');
  };

  $('.decrease_quantity').click(function () {
    var element = $(this).siblings('.quantity_value');
  ...
  $('.batch_form').submit(function (event) {
    var form = $('.batch_form');
    $(document).find('.quantity_value').each(function (index) {
      var value = $(this).text();
      var product_id = $(this).data('product_id');
      var batch_product_id = $(this).data('batch_product_id') || '';

      if (value === '0' && batch_product_id === '') {
        return;
      }

      form.append("<input name='batch[batches_products_attributes][".concat(index, "][quantity]' ") + "value='".concat($(this).text(), "' type='hidden'>"));
      form.append("<input name='batch[batches_products_attributes][".concat(index, "][product_id]' ") + "value='".concat($(this).data('product_id'), "' type='hidden'>"));
      form.append("<input name='batch[batches_products_attributes][".concat(index, "][id]' ") + "value='".concat($(this).data('batch_product_id') || '', "' type='hidden'>"));
    });
  });
});
...
/******/ });
//# sourceMappingURL=application-c4cbc9a77293981e6e23.js.map